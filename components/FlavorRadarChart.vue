...
import type { FlavorProfile } from '~/types/beer'
  ...
  <div class="relative" :style="{ width: `${size}px`, height: `${size}px` }">
    <svg :width="size" :height="size" class="overflow-visible">
      ...
      <circle
        ...
        :key="`point-${index}`"
        :cx="center + (flavorProfile[dataKeys[index]] / 100 * maxRadius) * Math.cos((Math.PI * 2 / 6) * index - Math.PI / 2)"
        :cy="center + (flavorProfile[dataKeys[index]] / 100 * maxRadius) * Math.sin((Math.PI * 2 / 6) * index - Math.PI / 2)"
        r="4"
        fill="#fbbf24"
        class="transition-all duration-500"
      />
