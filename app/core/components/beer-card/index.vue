...
<template>
  <UiCard class="group relative overflow-hidden card-hover cursor-pointer" @click="handleCardClick">
    ...
    <div class="relative aspect-[3/4] overflow-hidden">
      ...
      <!-- Gradient Overlay -->
      <div
        class="absolute inset-0 bg-gradient-to-t from-midnight-950 via-midnight-950/20 to-transparent"
      />

      <!-- Badges -->
      <div class="absolute top-3 left-3 flex flex-col gap-2">
        <UiBadge v-if="beer.isNew" variant="amber" class="flex items-center gap-1">
      ...
      <div class="absolute bottom-0 left-0 right-0 p-4">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            <div
              class="flex items-center gap-1 px-2 py-1 rounded-lg bg-midnight-900/80 backdrop-blur-sm"
            >
    ...
    <UiCardContent class="p-4">
      <!-- Style Badge -->
      <div class="mb-2">
        <UiBadge variant="secondary" class="text-xs">{{ beer.style }}</UiBadge>
      </div>

      <!-- Name & Brewery -->
      <h3
        class="text-lg font-bold text-foam-50 group-hover:text-amber-400 transition-colors line-clamp-1"
      >
        {{ beer.name }}
      </h3>
