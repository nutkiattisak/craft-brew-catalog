...
import type { Beer, FilterTag, SortOption } from '~/types/beer'
import { mockBeers } from '~/data/beers'
...
export function useBeerStore() {
    ...
    if (activeFilter.value !== 'all') {
      result = result.filter(beer => {
        switch (activeFilter.value) {
          ...
          case 'dark':
            return beer.style.toLowerCase().includes('stout') ||
  ...
  const featuredBeers = computed(() =>
    beers.value.filter(beer => beer.isFeatured)
  )

  const newBeers = computed(() =>
    beers.value.filter(beer => beer.isNew)
  )
